...
public class GeneratorDenoising {
    ...
    public int getRandomIndex(int countImages) {
        Random random = new Random();
        int r = random.nextInt(countImages);
        return r;
    }
    ...
    public void trainGeneratorWithRandomTrueImage(MnistData mnistData, int digit) {
        int countImages = mnistData.getTrainingInputs().rows();
        DataSet data = new DataSet(randomInput.getRandomInput().reshape(1, RANDOM_SIZE), mnistData.getIndexImage(getRandomIndex(countImages), digit).reshape(1, IMG_SIZE * IMG_SIZE));
        model.fit(data);
    }
